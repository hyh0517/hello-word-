<template>
	<view class="review-container">
		...
		<view class="word-card">
			<text class="word-text">{{ wordData.translations[0].translation }}</text>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async initOptions() {
				this.options = []; // 清空选项数组
				this.wait_options = [this.wordData.word]
				while (this.wait_options.length < 4) {
					await this.getOptions();
				}
				this.options = this.shuffleArray(this.wait_options); // 打乱选项顺序
			},
			...
			async getOptions() {
				await myRequest({
					...
					name: '获取选项',
					success: (response) => {
						console.log('选项请求成功:', response.data.option_en);
						this.wait_options.push(response.data.option_en);
					},
				});
			},
			// 打乱数组顺序
			...
			// 选择选项
			selectOption(index) {
				if (this.options[index] === this.wordData.word) { // 检查选择的英文单词是否正确
					uni.showToast({
						title: '答案正确',
						icon: 'none'
					});
